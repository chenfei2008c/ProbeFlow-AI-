...
- 进行中的文字场次首次启用语音时，沿用 `/api/participant/consent` 提交当前版本、`mode:voice`、`processing:true`、`permanent:true`；页面不在接受前申请麦克风或创建语音轮次。原文字同意记录保留。已有语音授权的场次逐轮选择文字时，直接提交 `input_mode:text`，不覆盖会话的语音授权；页面草稿在同页切换期间保留，不属于正式档案或跨刷新恢复承诺。
- 前端在录音、未完成上传、识别和待确认期间禁止回答方式切换。`rerecord` 经明确确认且服务端接受后，才清理对应轮次的本机暂存块；恢复上传时重新读取暂存清单。暂停先停止录音并释放麦克风，已有块仍保留以供恢复，正式音频和文字不受影响。
- 同页恢复上传或重新回答前等待原录音器的最后事件和本机写入完成，再读取清单或清理缓存。尚未 finalize 的轮次只允许 voice 且状态为 recording／uploading 时提交；superseded 等其他状态返回 TURN_NOT_RECORDING，迟到请求不能重新创建识别任务。已经 finalize 的同清单幂等响应不变。
- 进行中及已结束场次撤回均需两次确认；任一取消均不发送撤回请求。录音中的撤回等待最后录音事件和本机缓存写入完成，不等待上传成功；API 失败保留本机材料，成功后清理本场轮次（包含尚未轮询到的新录音）的暂存。服务器成功与浏览器清理结果分别展示，本机清理失败只能重试本机清理，不重新发送已成功的撤回。当前清理重试入口保留在结果页，离开或刷新前应完成；不承诺能远程擦除其他设备的缓存。
...
- 64,000 字符保护上限同样检查结构修复请求，包含错误输出和修复说明；超限返回 REPORT_CONTEXT_TOO_LARGE，不发送该次请求、不预占或结算该次费用、不截断来源。
- 人工重试 REPORT_INVALID，或因错误输出使修复请求超限的 REPORT_CONTEXT_TOO_LARGE 时，只移除已记录的失败步骤及其修复结果的内部检查点；成功步骤与来源快照保留，既有费用账本不改写，重新请求继续独立记账。原始来源自身超限不清除已成功步骤，仍须调整处理能力或导出人工整理。未知计费重试仍须明确确认。
