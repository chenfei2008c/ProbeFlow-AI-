...
更新源码后，重新执行 `uv sync --frozen` 和前端构建；待访谈和后台任务结束，在原服务终端按 `Ctrl+C`，再运行 `./scripts/probeflow start` 并刷新页面。只构建前端不会更新正在运行的后端，可能导致上传等新接口返回 405。
...
1. 在研究列表点击“选择调研方案文件”，上传可复制文字的 PDF，或 `.docx`、`.xlsx`、`.pptx`（最多 10 MB）。
2. 阅读并勾选解析说明，点击“上传并生成研究草稿”。真实模式会自动整理目标、对象、主题和问题；模拟模式仅展示提取文本与示例主题。
3. 等待完成，展开“核对提取原文”，检查自动填入的研究设计及待核对事项，点击“发布新版本”后即可创建邀请。已有研究也可在编辑页上传新方案。

不需要手工重填文件中已有的信息。原文件请自行保留；系统保存提取文本和草稿。旧版 `.doc`／`.xls`／`.ppt` 需另存为新格式；扫描件及图片文字暂不识别。超过 40,000 字符的方案会提示拆分，不会静默截断。处理失败可重试；配置变化须重新同意，费用未知须先确认。导入或归档不改变原有永久保存规则。
