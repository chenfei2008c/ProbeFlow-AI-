...
export function ConsentGate({ mode, providers, onSubmit, study, mock, purpose = 'start' }: Props) {
  ...
  return <form className="consent-card" onSubmit={submit}>
    <div className="eyebrow">{purpose === 'renew' ? '更新后续处理授权' : '开始之前'}</div>
    <h1>你的选择始终有效</h1>
    <p className="lead">{purpose === 'renew' ? '访谈已经结束。处理说明有更新；接受新说明后，研究者可以继续处理现有资料和生成报告。此操作不会重新开始访谈。' : '这场访谈由 AI 主持。你可以跳过问题、随时暂停或结束；结束会保留已提交内容，撤回会删除本场资料。'}</p>
    ...
    <button className="button primary wide" disabled={!processing || !permanent}>{purpose === 'renew' ? '同意更新后的处理与保存说明' : mode === 'voice' ? '同意并进行设备检查' : '同意并开始文字访谈'}</button>
