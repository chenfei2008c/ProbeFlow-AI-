...
export function ParticipantPage({ config }: { config: Config }) {
  ...
  const load = useCallback(async () => {
    ...
    else if (next.session.processing_consent && next.session.permanent_consent) setScreen(current => current === 'opening' || current === 'consent' ? (next.session.mode === 'voice' ? 'device' : 'interview') : current)
  ...
  if (screen === 'consent') return <ParticipantFrame config={config}><div className="consent-wrap">{error && <ErrorPanel error={error} />}<div className="mode-switch"><button className={mode === 'voice' ? 'active' : ''} onClick={() => setMode('voice')}>语音访谈</button><button className={mode === 'text' ? 'active' : ''} onClick={() => setMode('text')}>文字访谈</button></div><ConsentGate mode={mode} providers={config.providers} study={detail?.study} mock={config.mode === 'mock'} onSubmit={consent} /></div></ParticipantFrame>
